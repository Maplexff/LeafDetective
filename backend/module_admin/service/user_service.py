...
class UserService:
    ...
    async def check_user_name_unique_services(cls, query_db: AsyncSession, page_object: UserModel):
        ...
        :return: 校验结果
        """
        print(f"Checking if user name '{page_object.user_name}' is unique")  # 打印用户名字
    ...
    @classmethod
    async def check_user_nickname_unique_services(cls, query_db: AsyncSession, page_object: UserModel):
        """
        校验用户名是否唯一service
        ...
        """
        user_id = -1 if page_object.user_id is None else page_object.user_id
        user = await UserDao.get_user_by_info(query_db, UserModel(nickName=page_object.nick_name))
        if user and user.user_id != user_id:
            return CommonConstant.NOT_UNIQUE
        return CommonConstant.UNIQUE
    ...
    async def add_user_services(cls, query_db: AsyncSession, page_object: AddUserModel):
        ...
        elif page_object.nick_name and not await cls.check_user_nickname_unique_services(query_db, page_object):
            raise ServiceException(message=f'新增用户{page_object.user_name}失败，昵称已存在')
